...
// FIXED: Remove useRequireAuth to avoid hook conflicts
...
import { WalletDetailSidebar } from '@/components/wallet/WalletDetailSidebar';
...
export default function ManualPage() {
  ...
  // FIXED: Use consistent authentication sources only
  const { user, isAuthenticated: authContextAuthenticated } = useAuth();
  const { sdk, isReady, isAuthenticated: sdkAuthenticated, canMakeRequests } = useSDK();
  const { toast } = useToast();
  
  // FIXED: Consolidated authentication state
  ...
  // FIXED: Consolidated authentication check with debugging
  const checkAuthenticationState = () => {
    if (typeof window === 'undefined') return false;
    
    console.log('=== AUTHENTICATION DEBUG ===');
    console.log('1. SDK Ready:', isReady);
    console.log('2. SDK Authenticated:', sdkAuthenticated);
    console.log('3. Auth Context Authenticated:', authContextAuthenticated);
    console.log('4. User Object:', user);
    console.log('5. Can Make Requests:', canMakeRequests);
    ...
    console.log('6. LocalStorage auth_token:', authToken ? 'EXISTS' : 'MISSING');
    console.log('7. LocalStorage user_id:', userId || 'MISSING');
    console.log('8. LocalStorage ekyc_status:', ekycStatus);
    console.log('========================');
    ...
    const isAuth = !!(
      authToken && 
      userId && 
      (authContextAuthenticated || sdkAuthenticated) && 
      user?.user_id
    );
    
    console.log('9. FINAL AUTH STATE:', isAuth);
  ...
  useEffect(() => {
      if (isReady) {
        const authData = checkAuthenticationState();
        ...
        if (authData && typeof authData === 'object') {
          setAuthState(prev => ({
            ...
            ekycStatus: authData.ekycStatus,
    ...
    }, [isReady, authContextAuthenticated, sdkAuthenticated, user, router]);
  ...
  const calculateWalletTotalValue = async (wallet: WalletData): Promise<{ totalValue: number; totalTokens: number }> => {
    if (!canMakeRequests) {
      console.log('SDK not ready for balance calculation');
      return { totalValue: 0, totalTokens: 0 };
    }

    // Check cache first
    const cachedValue = getCachedBalance(wallet.bw_id);
    if (cachedValue !== null) {
      // For cached values, estimate token count from chains
      const estimatedTokens = (wallet.active_chains || 0) + (wallet.total_tokens || 0);
      return { totalValue: cachedValue, totalTokens: estimatedTokens };
    }

    console.log(`Calculating balance for wallet: ${wallet.wallet_name} (bw_id: ${wallet.bw_id})`);

    try {
      // Primary method: Get total balance from API
      const result = await calculateFromTotalAPI(wallet);
      if (result.success) {
        cacheBalance(wallet.bw_id, result.totalValue);
        return { totalValue: result.totalValue, totalTokens: result.totalTokens };
      }

      // Fallback method: Individual chain calculations
      const chainResult = await calculateFromChainAPI(wallet);
      if (chainResult.success) {
        cacheBalance(wallet.bw_id, chainResult.totalValue);
        return { totalValue: chainResult.totalValue, totalTokens: chainResult.totalTokens };
      }

      // Final fallback: Backend data
      const fallbackValue = wallet.total_value_usd || 0;
      const fallbackTokens = wallet.total_tokens || 0;
      cacheBalance(wallet.bw_id, fallbackValue);
      
      console.log(`Using backend fallback: $${fallbackValue}, ${fallbackTokens} tokens`);
      return { totalValue: fallbackValue, totalTokens: fallbackTokens };
  ...
  // Helper function for total API calculation
  const calculateFromTotalAPI = async (wallet: WalletData): Promise<{ success: boolean; totalValue: number; totalTokens: number }> => {
    try {
      ...
      if (!balanceResponse?.success || !balanceResponse?.data?.chains) {
        console.log('Balance API response invalid');
        return { success: false, totalValue: 0, totalTokens: 0 };
      }

      console.log('Processing chains from balance response');
      let totalValue = 0;
      let totalTokens = 0;
      
      Object.values(balanceResponse.data.chains).forEach((chainBalance: any) => {
        // Count and add native token
        totalTokens += 1;
        if (chainBalance.native_balance) {
          const nativeBalance = parseFloat(chainBalance.native_balance.balance || '0');
          if (nativeBalance > 0) {
            totalValue += chainBalance.native_balance.value_usd || 0;
          }
        }
        
        // Count and add other tokens
        if (chainBalance.tokens && Array.isArray(chainBalance.tokens)) {
          totalTokens += chainBalance.tokens.length;
          
          chainBalance.tokens.forEach((token: any) => {
            const tokenBalance = parseFloat(token.balance || '0');
            if (tokenBalance > 0) {
              if (token.symbol === 'USDC') {
                totalValue += token.value_usd || tokenBalance;
              } else {
                totalValue += token.value_usd || 0;
              }
            }
          });
        }
      ...
      console.log(`Total API result: $${totalValue}, ${totalTokens} tokens`);
      return { success: true, totalValue, totalTokens };
      
    } catch (error) {
      console.error('Total API calculation failed:', error);
      return { success: false, totalValue: 0, totalTokens: 0 };
    }
  };

  // Helper function for individual chain API calculation
  const calculateFromChainAPI = async (wallet: WalletData): Promise<{ success: boolean; totalValue: number; totalTokens: number }> => {
    ...
    try {
      ...
      for (const chain of wallet.chains) {
        ...
        try {
          ...
          if (chainBalance?.success && chainBalance?.data) {
            totalValue += chainBalance.data.total_value_usd || 0;
            // Estimate token count per chain
            totalTokens += chainBalance.data.token_count || 1;
          }
        } catch (chainError) {
          console.error(`Chain ${chain.chain_id} balance failed:`, chainError);
          continue;
        }
      }

      if (totalValue > 0) {
        console.log(`Chain API result: $${totalValue}, ${totalTokens} tokens`);
        return { success: true, totalValue, totalTokens };
      }
      ...
      console.error('Chain API calculation failed:', error);
      return { success: false, totalValue: 0, totalTokens: 0 };
    }
  };
  ...
  const loadWallets = async () => {
    if (!canMakeRequests || !authState.userId) return;
      ...
      if (response?.success && response?.data?.wallets) {
        ...
        const walletsList = await Promise.all(
          response.data.wallets.map(async (wallet: any, index: number) => {
            ...
            const { totalValue, totalTokens } = await calculateWalletTotalValue(walletObj);

            const finalWallet = {
              ...walletObj,
              total_value_usd: totalValue,
              calculated_token_count: totalTokens
            };
            
            console.log(`✅ Final wallet data:`, {
              name: finalWallet.wallet_name,
              bw_id: finalWallet.bw_id,
              calculated_value: totalValue,  // ← FIX: pakai variable yang benar
              calculated_tokens: totalTokens, // ← FIX: pakai variable yang benar
  ...
  // FIXED: Load wallets only after authentication is confirmed
  useEffect(() => {
    if (authState.isInitialized && authState.isAuthenticated && canMakeRequests) {
      loadWallets();
    }
  }, [authState.isInitialized, authState.isAuthenticated, canMakeRequests]);

  // FIXED: Show loading while authentication is being determined
  if (!authState.isInitialized || !isReady) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <LoadingSpinner size="lg" />
        <div className="ml-4 text-gray-600">
          {!isReady ? 'Initializing SDK...' : 'Verifying authentication...'}
        </div>
      </div>
    );
  }

  // FIXED: Don't render anything if redirecting
  ...
  // eKYC check - block access if not approved
  if (authState.ekycStatus !== 'APPROVED') {
    return (
          ...
          <div className="max-w-4xl mx-auto">
            <div className="bg-white rounded-lg shadow-md p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Please complete your eKYC verification to access wallet management features
              </p>
              <a
                href="/ekyc"
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          ...
                          <span>{wallet.calculated_token_count || wallet.total_tokens || 0} Tokens</span>
                        </div>
                      </div>
                    </div>
