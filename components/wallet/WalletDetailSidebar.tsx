//Path: components/wallet/WalletDetailSidebar.tsx
        ...
        <div className="flex border-b flex-shrink-0">
          ...
          >
            Tokens ({(() => {
              ...
              // Add native tokens (1 per chain)
              walletAddresses.forEach(address => {
                uniqueTokens.add(`${address.chain_id}-native`);
              });
              
              // Add imported tokens
              Object.entries(tokensByChain).forEach(([chainId, tokens]) => {
                (tokens as any[]).forEach((token: any) => { 
                  uniqueTokens.add(`${chainId}-${token.contract_address || token.symbol}`);
                });
              });
              
              // Add balance tokens (yang belum di-import)
              walletAddresses.forEach(address => {
                const chainBalance = chainBalances[address.chain_id];
                if (chainBalance?.tokens) {
                  chainBalance.tokens.forEach((balanceToken: any) => {
                    uniqueTokens.add(`${address.chain_id}-${balanceToken.contract_address || balanceToken.symbol}`);
                  });
                }
              });
