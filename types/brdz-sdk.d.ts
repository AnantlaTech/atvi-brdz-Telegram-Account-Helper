...
declare module 'anantla_sdk' {
  ...
  // NEW: Twitter Bot Interfaces
  interface TwitterVerificationData {
    user_id: number;
    twitter_username: string;
  }

  interface TwitterVerifyData {
    user_id: number;
    verification_code: string;
  }

  interface TwitterMentionRewardData {
    user_id: number;
    tweet_id: string;
    tweet_text: string;
    action_type?: string;
  ...
  interface TwitterRewardsHistoryParams {
    limit?: number;
    offset?: number;
    status?: 'pending' | 'completed' | 'failed';
    action_type?: 'verification' | 'mention' | 'retweet';
    chain?: 'sepolia' | 'amoy' | 'neon';
  }

  interface TwitterMintRewardData {
    chain: 'sepolia' | 'amoy' | 'neon';
    address: string;
    amount?: number; // optional, defaults to 10
  }

  interface TwitterMentionSearchParams {
    bot_username: string;
    max_results?: number;
  }

  // NEW: Privy Authentication Interfaces
  interface PrivyLoginData {
    privy_token: string;
  }

  interface PrivyLinkData {
    privy_token: string;
  }
  ...
  // Twitter Bot Module (16 methods) - UPDATED WITH MINT REWARD
  twitter: {
    // Verification Functions
    initiateVerification: (data: TwitterVerificationData) => Promise<any>;
    verifyAccount: (data: TwitterVerifyData) => Promise<any>;
    getVerificationStatus: (userId: string) => Promise<any>;

    // Reward Functions
    mintReward: (data: TwitterMintRewardData) => Promise<any>; // NEW: Added mint reward function
    processMentionReward: (data: TwitterMentionRewardData) => Promise<any>;
    getRewardsHistory: (userId: string, params?: TwitterRewardsHistoryParams) => Promise<any>;
    checkDailyLimits: (userId: string) => Promise<any>;

    // Twitter API Integration
    searchMentions: (params: TwitterMentionSearchParams) => Promise<any>;
    getTwitterUserInfo: (username: string) => Promise<any>;
    getHealthStatus: () => Promise<any>;

    // Webhook Management (Admin Only)
    getWebhookStatus: () => Promise<any>;
    testWebhookEvent: (data: { test_event_type: string; test_payload: any }) => Promise<any>;
    getWebhookLogs: (params?: { limit?: number; event_type?: string; status?: string }) => Promise<any>;

    // Static Pages
    getOAuthCallback: () => Promise<any>;
    ...
    // NEW: Privy Module (8 methods)
    privy: {
      loginWithPrivy: (data: PrivyLoginData) => Promise<any>;
      linkPrivyAccount: (data: PrivyLinkData) => Promise<any>;
      getPrivyProfile: () => Promise<PrivyProfile>;
      getUserPrivyProfile: (userId: string) => Promise<PrivyProfile>;
      linkWalletAddress: (data: PrivyWalletData) => Promise<any>;
      disconnectPrivyAuth: (data: PrivyDisconnectData) => Promise<any>;
      getHealthStatus: () => Promise<any>;
